...
function NewItemDetailPanelInner({ item, onClose }: { item: NewItem; onClose: () => void }) {
  ...
  const shareUrl = www ?? item.news?.url ?? item.sources.find((s) => s.url)?.url ?? 'https://mitatanaan.fi/uutta-helsingissa'
          ...
          <div className="p-6 space-y-5">
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                </button>                <button onClick={() => { navigator.clipboard.writeText(`${shareText}\n${shareUrl}`); setCopied(true); setTimeout(() => setCopied(false), 2000) }}
