...
// On-demand rich Google profile for ONE activity, read when its detail card is
// opened. Keeps the /api/activities list response lean — the heavy fields
// (attributes, rating distribution, popular times) load only on tap.
export async function GET(req: NextRequest) {
  const raw = (req.nextUrl.searchParams.get('key') || '').toLowerCase().trim()
  if (!raw) return NextResponse.json({ google: null })
  if (!isSupabaseConfigured() || !supabase) return NextResponse.json({ google: null })

  const { data, error } = await supabase
    .from('venue_ratings')
    .select('google_raw, review_count, google_rating')
    .eq('venue_key', `act:${raw}`)
    .maybeSingle()
  ...
  return NextResponse.json({
    google: {
      ...
      // available_attributes is grouped { amenities:[...], accessibility:[...], ... }
